...
class MineDojoSim(gym.Env):
    ...
    def save_world(self, action: Optional[dict] = None):
        """Flush all world data to disk via the /save-all Minecraft command.
        ...
        Return:
            A tuple (obs, reward, done, info)
        """
        return self._cmd_executor.save_world(action)

    def save_snapshot(self, save_path: str):
        """Save a snapshot of the current world to the given directory.

        Flushes world data to disk via /save-all, then copies the active
        Minecraft world save directory to *save_path*.

        Args:
            save_path: Destination directory path for the world snapshot.

        Raises:
            RuntimeError: If the world directory cannot be located.
        """
        import shutil
        import time
        import pathlib
        ...
        # Flush world to disk.
        self.save_world()
        time.sleep(2)  # Allow disk I/O to complete.

        # Locate the active world directory.
        # Minecraft saves are stored in instance's run directory under "saves/".
        # The world folder has a UUID-based name generated by Malmo.
        bridge_env = self._bridge_env
        if not bridge_env._instances:
            raise RuntimeError("No active Minecraft instance to save from.")

        instance = bridge_env._instances[0]
        run_dir = instance._runDir
        saves_dir = pathlib.Path(run_dir) / "saves"
